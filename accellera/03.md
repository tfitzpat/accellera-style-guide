...
By default, Kramdown will automatically create an ID for each header, such that it can be referenced (see also [3.1.10](#3110-links-and-references)). Kramdown supports the definition of a user-defined ID as shown below. Note that in this case, the user should reference to this user-defined ID when when creating a cross-reference. The use of user-defined IDs is *not recommended*, as GitHub will not render these links correctly in the source files.

<pre><code># This is a first-level heading  {#my-first-id}

{% raw %}## This is a second-level heading with trailing hashes ##  {#my-second-id}{% endraw %}
</code></pre>
...
Ordered lists are started by using a number followed by a period, a space and then the list item text. According to the IEEE-SA style manual, ordered items use letters a), b), c), etc. If a subdivision of the items is necessary, 1), 2), 3); i), ii), iii); dashed subdivision items, etc., should be used to form a tiered list.
...
This sentence is part of the same line.
   ...
   2. second subitem will get number 1.
...
Three consecutive backtick characters are used to mark the start and end of a code block. An optional language identifier can be added to enable syntax highlighting in the code block. The example below shows a C++ code block using the `cpp` language identifier. 
...
#### 3.1.10.2 Internal links / cross references
...
```
See [Clause 1](01.html) for more information.
```
...
To cross reference a subclause, use the header ID of that subclause. The default ID is a *slug* of the header, containing the section number and title. For example, for section 2.1 called “2.1 GitHub repository setup” the *slug* is `21-github-repository-setup`. In case the section is located in a different file, the file needs to be specified as well.
